...
docker exec -it broker-1 /opt/kafka/bin/kafka-topics.sh --command-config /etc/kafka/kraft/client-admin-config.properties --bootstrap-server broker-1:19092,broker-2:19092,broker-3:19092 --create --topic push.tokens --partitions 3 --replication-factor 3 --config cleanup.policy=compact --config retention.ms=86400000
docker exec -it broker-1 /opt/kafka/bin/kafka-topics.sh --command-config /etc/kafka/kraft/client-admin-config.properties --bootstrap-server broker-1:19092,broker-2:19092,broker-3:19092 --create --topic ok.web --partitions 3 --replication-factor 3 --config retention.ms=86400000
docker exec -it broker-1 /opt/kafka/bin/kafka-topics.sh --command-config /etc/kafka/kraft/client-admin-config.properties --bootstrap-server broker-1:19092,broker-2:19092,broker-3:19092 --create --topic ok.mail --partitions 3 --replication-factor 3 --config retention.ms=86400000
docker exec -it broker-1 /opt/kafka/bin/kafka-topics.sh --command-config /etc/kafka/kraft/client-admin-config.properties --bootstrap-server broker-1:19092,broker-2:19092,broker-3:19092 --create --topic ok.push --partitions 3 --replication-factor 3 --config retention.ms=86400000
